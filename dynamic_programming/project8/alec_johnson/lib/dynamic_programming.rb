...
class DPProblems
  ...
  def knapsack_helper(index, capacity, weights, values)
    # returns the highest possible value using only the first **index** items
    # in the array from the knapsack problem
    return -Float::INFINITY if capacity < 0 || index < 0
    return 0 if capacity == 0 || index == 0
    return @knapsack_matrix[capacity][index] if @knapsack_matrix[capacity][index]
    take_item = knapsack_helper(index - 1, capacity - weights[index], weights, values) + values[index]
    dont_take_item = knapsack_helper(index - 1, capacity, weights, values)
    [take_item, dont_take_item].max
  end
